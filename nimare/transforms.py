...
def t_to_z(t_maps, sample_sizes):
    assert t_maps.shape[0] == len(sample_sizes)
    res = [t_to_z(t_map, sample_sizes-1) for t_map in tmaps]
    res = np.vstack(res)
    return res


def se_to_var(se_maps, sample_sizes):
    assert se_maps.shape[0] == len(sample_sizes)
    res = [t_to_z(t_map, sample_sizes-1) for t_map in tmaps]
    res = np.vstack(res)
    return res
