...
def test_CBMRInference(testdata_cbmr_simulated):
    ...
    """Unit test for CBMR estimator."""
    dset = standardize_field(dataset=testdata_cbmr_simulated, metadata=["sample_sizes", 'avg_age'])
    cbmr = CBMREstimator(group_names=['diagnosis', 'drug_status'], moderators=['standardized_sample_sizes', 'standardized_avg_age'], spline_spacing=10, model='Poisson', penalty=False, lr=1e-1, tol=1e6, device='cuda')
    cbmr_res = cbmr.fit(dataset=dset)
    inference = CBMRInference(CBMRResults=cbmr_res, t_con_group=False, t_con_moderator=[[1,0]], device='cuda') 
